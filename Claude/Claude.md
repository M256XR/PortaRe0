...
- ファン：RP2040 #1 GP26（FAN_PWM）→ BSS138 → J6（2pin JST PH）経由で制御（Cubie A7Z内蔵コネクタは不使用）
...
### やらないこと
- `.kicad_sch` ファイルの直接編集（シンボル配置・配線・ネットラベル追加等）
- 複雑なPythonスクリプトによる回路図の自動生成
- ユーザーが自分でやると言っている作業の代行

### 基本スタンス
- 回路設計はユーザーがKiCad GUIで行う
- Claudeは「何をどう接続するか」を提案・確認する役割
- 必要なシンボルのC番号とコマンドを提示し、取得はClaudeが実行できる
